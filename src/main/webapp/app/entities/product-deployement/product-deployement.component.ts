...
export default defineComponent({
  ...
  setup() {
    ...
    const addModuleToDetail = async () => {
      ...
      const moduleVersion = availableModuleVersions.value.find(mv => mv.id === Number.parseInt(selectedModuleVersionId.value));
    ...
    const getInfraComponentVersionsForSelectedDetail = () => {
      ...
      const productVersion = productVersions.value.find(pv => pv.id === selectedDetailForConfiguration.value.productVersion.id);
    ...
    onMounted(async () => {
      ...
      try {
        ...
        console.log('Initialisation terminée - Modules:', moduleOptions.value.length, 'ModuleVersions:', moduleVersionOptions.value.length);
      } catch (error) {
