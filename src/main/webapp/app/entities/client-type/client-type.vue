<template>
  <div class="client-type-container section pt-5">
    <div class="navigation-bar d-flex align-items-center justify-content-between mb-4">
      <div class="d-flex align-items-center">
        ...
        <h5 id="page-heading" class="m-0 font-weight-bold" data-cy="ClientTypeHeading">
    ...
    <div class="card" v-if="clientTypes && viewMode === 'list'">
      <div class="table-responsive">
        <table class="table table-hover mb-0" aria-describedby="clientTypes">
          ...
          <tbody>
            <tr v-for="clientType in paginatedClientTypes" :key="clientType.id" data-cy="entityTable" class="align-middle">
              <td>
                <template v-if="clientType.isEditing">
                  <input v-model="clientType.type" type="text" class="form-control-borderless" />
                </template>
                <template v-else>
                  {{ clientType.type }}
                </template>
              ...
              <td class="text-center">
                <div class="btn-group">
                  <template v-if="clientType.isEditing">
                    <div class="icon-container save-container" @click="saveClientType(clientType)" title="Enregistrer">
                    ...
                    <div class="icon-container cancel-container" @click="cancelEdit(clientType)" title="Annuler">
                  ...
                  <template v-else>
                    <div class="action-icons">
                      <div class="icon-container edit-container" @click="editClientType(clientType)" title="Modifier">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="16"
                          height="16"
                      ...
                      <div class="icon-container delete-container" @click="prepareRemove(clientType)" title="Supprimer">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="26"
                          height="26"
            ...
            <tr v-if="showAddRow" class="add-row">
              ...
              <td class="text-center">
                <div class="action-icons">
                  <div class="icon-container save-container" @click="saveNewClientType" title="Enregistrer">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" class="icon-save">
                      <path
                        d="M144 480C64.5 480 0 415.5 0 336c0-62.8 40.2-116.2 96.2-135.9c-.1-2.7-.2-5.4-.2-8.1c0-88.4 71.6-160 160-160c59.3 0 111 32.2 138.7 80.2C409.9 102 428.3 96 448 96c53 0 96 43 96 96c0 12.2-2.3 23.8-6.4 34.6C596 238.4 640 290.1 640 352c0 70.7-57.3 128-128 128l-368 0zm79-217c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l39-39L296 392c0 13.3 10.7 24 24 24s24-10.7 24-24l0-134.1 39 39c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-80-80c-9.4-9.4-24.6-9.4-33.9 0l-80 80z"
                        fill="currentColor"
                  ...
                  <div class="icon-container cancel-container" @click="cancelNewClientType" title="Annuler">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" class="icon-cancel">
                      <path
                        d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"
                        fill="currentColor"
    ...
    <!-- Card View -->
    <div v-if="clientTypes && viewMode === 'card'" class="card-view-container">
      <div class="row">
        <div v-for="clientType in paginatedClientTypes" :key="clientType.id" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 client-type-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold">
                <router-link class="text-primary" :to="{ name: 'ClientTypeView', params: { clientTypeId: clientType.id } }">
                  #{{ clientType.id }}
                </router-link>
              </h6>
              <div class="dropdown">
                <button class="btn btn-sm btn-light" @click="toggleDropdown(clientType)">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                ...
                <div class="dropdown-menu dropdown-menu-right" :class="{ show: clientType.showDropdown }">
                  <a class="dropdown-item" @click="editClientType(clientType)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                  ...
                  <a class="dropdown-item" @click="prepareRemove(clientType)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                  ...
                  <router-link class="dropdown-item" :to="{ name: 'ClientTypeView', params: { clientTypeId: clientType.id } }">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
            ...
            <div class="card-body">
              <template v-if="clientType.isEditing">
                <div class="form-group">
                  <label>Type</label>
                  <input v-model="clientType.type" type="text" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Date de création</label>
                  <input v-model="clientType.createDate" type="date" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Date de mise à jour</label>
                  <input v-model="clientType.updateDate" type="date" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Notes</label>
                  <textarea v-model="clientType.notes" class="form-control" rows="3"></textarea>
                </div>
                <div class="d-flex justify-content-end mt-3">
                  <button class="btn btn-sm btn-secondary mr-2" @click="cancelEdit(clientType)">Annuler</button>
                  <button class="btn btn-sm btn-primary" @click="saveClientType(clientType)">Enregistrer</button>
                </div>
              </template>
              <template v-else>
                <div class="client-type-info">
                  <div class="info-row">
                    ...
                    <span class="info-value">{{ clientType.type }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Date de création:</span>
                    <span class="info-value">{{ clientType.createDate }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Date de mise à jour:</span>
                    <span class="info-value">{{ clientType.updateDate }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Notes:</span>
                    <span class="info-value notes-text">{{ clientType.notes }}</span>
                  </div>
                </div>
              </template>
            </div>
        ...
        <div v-if="showAddRow" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 client-type-card new-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold text-primary">Nouveau type</h6>
            </div>
            <div class="card-body">
              <div class="form-group">
                <label>Type</label>
                <input v-model="newClientType.type" type="text" class="form-control" placeholder="Type" />
              </div>
              <div class="form-group">
                <label>Date de création</label>
                <input v-model="newClientType.createDate" type="date" class="form-control" />
              </div>
              <div class="form-group">
                <label>Date de mise à jour</label>
                <input v-model="newClientType.updateDate" type="date" class="form-control" />
              </div>
              <div class="form-group">
                <label>Notes</label>
                <textarea v-model="newClientType.notes" class="form-control" rows="3" placeholder="Notes"></textarea>
              </div>
              <div class="d-flex justify-content-end mt-3">
                <button class="btn btn-sm btn-secondary mr-2" @click="cancelNewClientType">Annuler</button>
                <button class="btn btn-sm btn-primary" @click="saveNewClientType">Enregistrer</button>
              </div>
            </div>
          </div>
        </div>
