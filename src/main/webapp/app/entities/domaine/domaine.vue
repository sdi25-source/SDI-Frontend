<template>
  <div class="domaine-container section pt-5">
    <div class="navigation-bar d-flex align-items-center justify-content-between mb-4">
      <div class="d-flex align-items-center">
        ...
        <h5 id="page-heading" class="m-0 font-weight-bold" data-cy="DomaineHeading">
    ...
    <!-- Vue Tableau -->
    <div class="card" v-if="domaines && viewMode === 'list'">
    ...
    <!-- Vue Cartes -->
    <div v-if="domaines && viewMode === 'card'" class="card-view-container">
      <div class="row">
        <div v-for="domaine in paginatedDomaines" :key="domaine.id" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 domaine-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold">
                <router-link class="text-primary" :to="{ name: 'DomaineView', params: { domaineId: domaine.id } }">
                  #{{ domaine.id }}
                </router-link>
              </h6>
              <div class="dropdown">
            ...
            <div class="card-body">
              <template v-if="domaine.isEditing">
                <div class="form-group">
                  <label>Nom</label>
                  <input v-model="domaine.name" type="text" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Date de création</label>
                  <input v-model="domaine.createDate" type="date" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Date de mise à jour</label>
                  <input v-model="domaine.updateDate" type="date" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Notes</label>
                  <textarea v-model="domaine.notes" class="form-control" rows="3"></textarea>
                </div>
                <div class="d-flex justify-content-end mt-3">
                  <button class="btn btn-sm btn-secondary mr-2" @click="cancelEdit(domaine)">Annuler</button>
                  <button class="btn btn-sm btn-primary" @click="saveDomaine(domaine)">Enregistrer</button>
                </div>
              </template>
              <template v-else>
                <div class="domaine-info">
                  <div class="info-row">
                    ...
                    <span class="info-value">{{ domaine.name }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Créé le:</span>
                    <span class="info-value">{{ domaine.createDate }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Mis à jour le:</span>
                    <span class="info-value">{{ domaine.updateDate }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Notes:</span>
                    <span class="info-value notes-text">{{ domaine.notes }}</span>
                  </div>
                </div>
              </template>
            </div>
        ...
        <div v-if="showAddRow" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 client-type-card new-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold text-primary">Nouveau domaine</h6>
            </div>
            <div class="card-body">
              <div class="form-group">
                <label>Nom</label>
                <input v-model="newDomaine.name" type="text" class="form-control" placeholder="Type" />
              </div>
              <div class="form-group">
                <label>Date de création</label>
                <input v-model="newDomaine.createDate" type="date" class="form-control" />
              </div>
              <div class="form-group">
                <label>Date de mise à jour</label>
                <input v-model="newDomaine.updateDate" type="date" class="form-control" />
              </div>
              <div class="form-group">
                <label>Notes</label>
                <textarea v-model="newDomaine.notes" class="form-control" rows="3" placeholder="Notes"></textarea>
              </div>
              <div class="d-flex justify-content-end mt-3">
                <button class="btn btn-sm btn-secondary mr-2" @click="cancelNewDomaine">Annuler</button>
                <button class="btn btn-sm btn-primary" @click="saveNewDomaine">Enregistrer</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity" centered title-class="text-danger">
      <template #modal-title>
        <div class="d-flex align-items-center">
          ...
          <span
            id="sdiFrontendApp.domaine.delete.question"
            data-cy="domaineDeleteDialogHeading"
      ...
      <div class="modal-body">
        <p id="jhi-delete-domaine-heading" class="mb-0" v-text="t$('sdiFrontendApp.domaine.delete.question', { id: removeId })"></p>
      </div>
