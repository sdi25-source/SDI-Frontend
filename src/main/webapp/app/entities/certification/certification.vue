<template>
  <div class="certification-container section pt-5">
    ...
    <!-- Card View -->
    <div v-if="certifications && viewMode === 'card'" class="card-view-container">
      <div class="row">
        <div v-for="certification in paginatedCertifications" :key="certification.id" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 certification-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold">
                <router-link class="text-primary" :to="{ name: 'CertificationView', params: { certificationId: certification.id } }">
                  #{{ certification.id }}
                </router-link>
              </h6>
              <div class="dropdown">
                <button class="btn btn-sm btn-light" @click="toggleDropdown(certification)">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                ...
                <div class="dropdown-menu dropdown-menu-right" :class="{ show: certification.showDropdown }">
                  <a class="dropdown-item" @click="editCertification(certification)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                  ...
                  <a class="dropdown-item" @click="prepareRemove(certification)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                  ...
                  <router-link class="dropdown-item" :to="{ name: 'CertificationView', params: { certificationId: certification.id } }">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
            ...
            <div class="card-body">
              <template v-if="certification.isEditing">
                <div class="form-group">
                  <label>Nom</label>
                  <input v-model="certification.name" type="text" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Description</label>
                  <textarea v-model="certification.description" class="form-control" rows="3"></textarea>
                </div>
                <div class="form-group">
                  <label>Date de création</label>
                  <input v-model="certification.createDate" type="date" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Date d'expiration</label>
                  <input v-model="certification.expireDate" type="date" class="form-control" />
                </div>
                <div class="d-flex justify-content-end mt-3">
                  <button class="btn btn-sm btn-secondary mr-2" @click="cancelEdit(certification)">Annuler</button>
                  <button class="btn btn-sm btn-primary" @click="saveCertification(certification)">Enregistrer</button>
                </div>
              </template>
              <template v-else>
                <div class="certification-info">
                  ...
                  <div class="info-row">
                    ...
                    <span class="info-value notes-text">{{ certification.description }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Date de création:</span>
                    <span class="info-value">{{ formatDate(certification.createDate) }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Date d'expiration:</span>
                    <span class="info-value">{{ formatDate(certification.expireDate) }}</span>
                  </div>
                </div>
              </template>
            </div>
        ...
        <div v-if="showAddRow" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 certification-card new-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold text-primary">Nouvelle certification</h6>
            </div>
            <div class="card-body">
              <div class="form-group">
                <label>Nom</label>
                <input v-model="newCertification.name" type="text" class="form-control" placeholder="Nom" />
              </div>
              <div class="form-group">
                <label>Description</label>
                <textarea v-model="newCertification.description" class="form-control" rows="3" placeholder="Description"></textarea>
              </div>
              <div class="form-group">
                <label>Date de création</label>
                <input v-model="newCertification.createDate" type="date" class="form-control" />
              </div>
              <div class="form-group">
                <label>Date d'expiration</label>
                <input v-model="newCertification.expireDate" type="date" class="form-control" />
              </div>
              <div class="d-flex justify-content-end mt-3">
                <button class="btn btn-sm btn-secondary mr-2" @click="cancelNewCertification">Annuler</button>
                <button class="btn btn-sm btn-primary" @click="saveNewCertification">Enregistrer</button>
              </div>
            </div>
          </div>
        </div>
