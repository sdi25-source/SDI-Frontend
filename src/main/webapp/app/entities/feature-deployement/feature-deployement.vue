<template>
  <div>
    <h2 id="page-heading" data-cy="FeatureDeployementHeading">
      ...
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          ...
          <span v-text="t$('sdiFrontendApp.featureDeployement.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'FeatureDeployementCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('sdiFrontendApp.featureDeployement.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && featureDeployements && featureDeployements.length === 0">
      <span v-text="t$('sdiFrontendApp.featureDeployement.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="featureDeployements">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('sdiFrontendApp.featureDeployement.code')"></span></th>
            <th scope="row"><span v-text="t$('sdiFrontendApp.featureDeployement.createDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiFrontendApp.featureDeployement.updateDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiFrontendApp.featureDeployement.notes')"></span></th>
            <th scope="row"><span v-text="t$('sdiFrontendApp.featureDeployement.feature')"></span></th>
            <th scope="row"><span v-text="t$('sdiFrontendApp.featureDeployement.moduleDeployement')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="featureDeployement in featureDeployements" :key="featureDeployement.id" data-cy="entityTable">
            ...
            <td>{{ featureDeployement.createDate }}</td>
            ...
            <td>
              <div v-if="featureDeployement.moduleDeployement">
                <router-link
                  ...
                  >{{ featureDeployement.moduleDeployement.code }}</router-link
                >
              </div>
            </td>
            <td class="text-right">
