<template>
  <div class="product-line-container section pt-5">
      ...
      <div class="table-responsive">
        <table class="table table-hover mb-0" aria-describedby="productLines">
          ...
          <tr v-if="showAddRow" class="add-row">
            <td><input type="text" class="form-control-borderless" v-model="newProductLine.name" placeholder="Nom" /></td>
    ...
    <!-- Card View -->
    <div v-if="productLines && viewMode === 'card'" class="card-view-container">
      <div class="row">
        <div v-for="productLine in paginatedProductLines" :key="productLine.id" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 product-line-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold">
                <router-link class="text-primary" :to="{ name: 'ProductLineView', params: { productLineId: productLine.id } }">
                  #{{ productLine.id }}
                </router-link>
              </h6>
              <div class="dropdown">
                <button class="btn btn-sm btn-light" @click="toggleDropdown(productLine)">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                ...
                <div class="dropdown-menu dropdown-menu-right" :class="{ show: productLine.showDropdown }">
                  <a class="dropdown-item" @click="editProductLine(productLine)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                  ...
                  <a class="dropdown-item" @click="prepareRemove(productLine)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                  ...
                  <router-link class="dropdown-item" :to="{ name: 'ProductLineView', params: { productLineId: productLine.id } }">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
            ...
            <div class="card-body">
              <template v-if="productLine.isEditing">
                <div class="form-group">
                  <label>Nom</label>
                  <input v-model="productLine.name" type="text" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Notes</label>
                  <textarea v-model="productLine.notes" class="form-control" rows="3"></textarea>
                </div>
                <div class="d-flex justify-content-end mt-3">
                  <button class="btn btn-sm btn-secondary mr-2" @click="cancelEdit(productLine)">Annuler</button>
                  <button class="btn btn-sm btn-primary" @click="saveProductLine(productLine)">Enregistrer</button>
                </div>
              </template>
              <template v-else>
                <div class="product-line-info">
                  <div class="info-row">
                    ...
                    <span class="info-value">{{ productLine.name }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Notes:</span>
                    <span class="info-value notes-text">{{ productLine.notes }}</span>
                  </div>
                </div>
              </template>
            </div>
        ...
        <div v-if="showAddRow" class="col-md-4 col-lg-3 mb-4">
          <div class="card h-100 product-line-card new-card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0 font-weight-bold text-primary">Nouvelle ligne de produit</h6>
            </div>
            <div class="card-body">
              <div class="form-group">
                <label>Nom</label>
                <input v-model="newProductLine.name" type="text" class="form-control" placeholder="Nom" />
              </div>
              <div class="form-group">
                <label>Notes</label>
                <textarea v-model="newProductLine.notes" class="form-control" rows="3" placeholder="Notes"></textarea>
              </div>
              <div class="d-flex justify-content-end mt-3">
                <button class="btn btn-sm btn-secondary mr-2" @click="cancelNewProductLine">Annuler</button>
                <button class="btn btn-sm btn-primary" @click="saveNewProductLine">Enregistrer</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity" centered title-class="text-danger">
      ...
      <div class="modal-body">
        <p
          id="jhi-delete-productLine-heading"
          class="mb-0"
          v-text="t$('sdiFrontendApp.productLine.delete.question', { id: removeId })"
        ></p>
      </div>
